...
class ASTGeneration(CSlangVisitor):
    ...
    def visitShort_form(self, ctx: CSlangParser.Short_formContext):
        list=self.visit(ctx.short_identifier_list())
        if(len(list)>1):
            
            return [AttributeDecl(VarDecl(list[x], self.visit(ctx.whole_type()))) for x in range(0,len(list))]
        else:
            return AttributeDecl(VarDecl(list[0], self.visit(ctx.whole_type())))
    ...
    def visitFull_form(self, ctx: CSlangParser.Full_formContext):
        ...
        res = [AttributeDecl(VarDecl(id_list[i], var_type, expr_list[len(id_list)-i-1])

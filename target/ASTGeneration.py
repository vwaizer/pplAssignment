...
class ASTGeneration(CSlangVisitor):
    ...
    def visitClassdecl(self, ctx: CSlangParser.ClassdeclContext):
        if (ctx.classbody()):
            if (ctx.RARROW()):
                return ClassDecl((self.visit(ctx.id_type(1))), [self.visit(x) for x in ctx.classbody()], (self.visit(ctx.id_type(0))))
            else:
                
                return ClassDecl((self.visit(ctx.id_type(0))), [self.visit(x) for x in ctx.classbody()])
        else:
            # print("vao null")
    ...
    def visitShort_form(self, ctx: CSlangParser.Short_formContext):
        listv=self.visit(ctx.short_identifier_list())
        if(len(listv)>1):
            res=reduce(lambda x,y:x+[AttributeDecl(VarDecl(y, self.visit(ctx.whole_type())))],listv,[])
            print(*res)
            return list(res)
        else:
            return AttributeDecl(VarDecl(listv[0], self.visit(ctx.whole_type())))
        

    def visitShort_identifier_list(self, ctx: CSlangParser.Short_identifier_listContext):
        if (ctx.getChildCount() == 3):
            return  [self.visit(ctx.id_type())]+((self.visit(ctx.short_identifier_list())))
        return [self.visit(ctx.id_type())]

    def visitFull_form(self, ctx: CSlangParser.Full_formContext):
        id_list = [self.visit(ctx.id_type())]
        temp_list = self.visit(ctx.identifier_list())
        var_type = temp_list[-1]
        temp_list = temp_list[:-1]
        id_list = id_list + [x[0] for x in temp_list]
        expr_list = [self.visit(ctx.expr())] + [y[1] for y in temp_list]
        
        res = [AttributeDecl(VarDecl(id_list[i], var_type, expr_list[len(id_list)-i-1]) ) for i in range(len(id_list))]
        print(*res)
        return res

    def visitIdentifier_list(self, ctx: CSlangParser.Identifier_listContext):
        if (ctx.id_type()):
            return [(self.visit(ctx.id_type()), self.visit(ctx.expr()))] + self.visit(ctx.identifier_list())
    ...
    def visitExpression_list(self, ctx: CSlangParser.Expression_listContext):
        if (ctx.getChildCount() == 1):
            ...
            list = ctx.COMMA()
            return [self.visit(ctx.expr(x)) for x in range(len(list))]
    ...
    def visitExpr9(self, ctx: CSlangParser.Expr9Context):
        if (ctx.DOT()):
            ...
            return self.visit(ctx.expr10(0))
    ...
    def visitFunction_call(self, ctx: CSlangParser.Function_callContext):
        ...
        return CallExpr(self.visit(ctx.id_type()), self.visit(ctx.function_call_params()))
    def visitFunction_call_stmt(self,ctx:CSlangParser.Function_call_stmtContext):
        if(ctx.DOT()):
            return CallStmt(self.visit(ctx.id_type(0)),self.visit(ctx.id_type(1)), self.visit(ctx.function_call_params()))
    ...
    def visitConst_decl(self, ctx: CSlangParser.Const_declContext):
        list=self.visit(ctx.full_form_const())
        return list
    ...
    def visitFull_form_const(self, ctx: CSlangParser.Full_form_constContext):
        ...
        temp_list = temp_list[:-1]
        id_list = id_list + [x[0] for x in temp_list]
        expr_list = [self.visit(ctx.expr())] + [y[1]
                                                for y in temp_list]

        res = [AttributeDecl(ConstDecl(id_list[i], var_type, expr_list[len(id_list)-i-1])
    ...
    def visitFunction_call_param_list(self, ctx: CSlangParser.Function_call_param_listContext):
        if (ctx.getChildCount() == 3):
            return [self.visit(ctx.expr())] + self.visit(ctx.function_call_param_list())
        return [self.visit(ctx.expr())]
    ...
    def visitParameter_declare(self, ctx: CSlangParser.Parameter_declareContext):
        return VarDecl(self.visit(ctx.id_type()),self.visit(ctx.whole_type()))
